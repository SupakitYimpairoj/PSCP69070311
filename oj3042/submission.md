...
```text
โจทย์ให้เขียนโปรแกรมรับค่าจำนวนเต็มบวก แล้วให้หาว่า ตั้งแต่เลขที่กำหนดถึง 0  มีเลขไหนบ้างที่หาสิบลงตัวใให้พิม output ออกมาทั้งหมด

Input:
โปรแกรมรับจำนวนเต็มหนึ่งค่า n

Output:
แสดงเอาพุตออกมาเป็นบรรถัดเดียว คือผลว่าจากเลขที่กำหนด มีอะไรหารสิบได้บ้าง

Constraints:
ต้องเอาพุตออกมาบรรทัดเดียวเพราะชะนั้นจะต้องกำหนดค่าทุกอย่างให้อยู่ในปริ้นเดียว
```
...
```text
แผลนของผมคือเอา n มาหารเอาเศษไปเรื่อยๆเพราะถ้าเศษเป็นศูยน์แปลว่าหาร 10 ลงตัว ผมเลยเลือกใช้ while ลูปโดยให้ทำการลบไปทีละ 1 แล้วถ้าเลขนั้นหาร 10 ลงตัวให้ พิมพ์ n แล้วลงท้ายด้วย " " หรือเว้นว่างตามที่โจทย์กำหนด แล้ววนไปเรื่อยๆ จนเลขที่หาร 10 ได้หมดหรือก็คือถึง 0
```
...
```text
มีแก้ตรง n % 10 == 0 เพราะไม่ผ่าน prep 8 แล้วเค้าแนะนำให้ให้ใช้ not n แทน
```
...
```text
เพื่อดูว่าโปรแกรมรันตามต้องการจริงไหม
```
...
```text
ดูว่าระบบการคำนวนถูกต้องจริงๆใช่ไหม
```
