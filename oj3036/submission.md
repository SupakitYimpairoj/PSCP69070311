...
```text
โจทย์ต้องการให้ผมหาทางที่สั้นที่สุดจากเลข n  ไปเลข 1

Input:
โปรแกรมรับจำนวนเต็มหนึ่งค่า n

Output:
กำแพงที่ต้องพังเผื่อไปถึง 1 ที่น้อยที่สุด

Constraints:
ถ้า n เป็น 1 อยู่แล้วก็ไม่ต้องพังกำแพงตอบ 0 ได้เลย
```
...
Step 2: ตรวจว่า n == 1
Step 3: ถ้าจริง ให้พิมพ์ "0"
Step 4: ในกรณีอื่นให้มาหาชั้น
Step 5: ตำแหน่งของเลขในชั้นนี้

ก่อนถามai ผมรู้ว่ามันต้องใช้สูตรคำนวนในการหาการพังกำแพงที่สั้นที่สุด แต่ผมนึกไม่ออก
```
...
```text
วิธีสุดท้ายของผมตามแบบแรกแต่ตรง step 4 ผมได้สูตรคำนวนจาก ai มาซึ่งมันเป็นการหาชั้นโดยการถอด sqrt ของ n แล้วใช้ math.ceil ปัดขึ้นเพื่อให้ได้ชั้นของ n มา จากนั้นผมก็คิดต่อว่าว่าเราสามารถหาตำแหน่งของเลขด้วยการใช้สูตร n - (sqrt n ที่หามาเมื่อกี้(level) -1)**2 จากนั้นเป็นการหาการพังงกำแพงน้อยที่สุด ซึ่งผมนึกไม่ออก และได้ ai ช่วย ได้สูตรว่าถ้าเป็นคู่ 2 * level - 2 ถ้าเป็นคี่ 2 * level - 3 ห้องที่อยู่ในตำแหน่งต่างกัน จะมีจำนวนกำแพงที่ต้องพังเพื่อเดินขึ้นไปหา 1 ต่างกัน 1 กำแพงไปเรื่อยๆ
```
...
```text
เพื่อทดสอบว่าแนวคิดนี้ไม่พังออกนอกกรอบสามเหลี่ยมจริง
```
...
```text
ถ้าเป็นเลขที่ใหญ่ขึ้นมาโปรแกรมยังทำงานถูกต้อง
```
