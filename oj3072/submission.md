...
```text
โจทย์ให้นับสระในคำที่ให้มา

Input:
คำเพื่อให้นับสระ 1 บรรทัด

Output:
สรพที่นับได้แสดงเป็นอย่างเช่น a : 1

Constraints:
input เป็นไม่กำหนดว่าเป็นพิมพ์เล็กหรือใหญ่ ตอนรับอินพุทผมเลยจะใช้ .lower() เพื่อง่ายต่อการเปรียบเทียบและจะได้ไม่เกิดปัญหา
```
...
```text
Step 1: รับ input คำ
Step 2: ตรวจว่า a อยู่ในคำไหม
Step 3: ถ้าจริง ให้นับ a โดยใช้ count แล้วให้พิมพ์ a : {ผลรวม a ทั้งหมด}
Step 4: ทำซ่ำกับอื่นให้ครบ
```
...
```text
ทำเหมือนที่วางแผนไว้ก่อนหน้านี้
```
...
```text
ทดสอบว่าโปรแกรมใช้งานได้ถ้าเป็นคำที่ยาวขึ้น
```

Input:

```text
uaosidopaiocihugiuygwidasldnanca
```
