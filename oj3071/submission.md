...
```text
โจทย์ให้หาว่า x ในช่วง a b มีกี่จำนสนที่หาร d แล้วได้เศษ r

Input:
4 บรรทัด a, b, d, r

Output:
จำนวนรวมทั้งหมดว่ามีเลขกี่ตัวที่ d แล้วได้ r

Constraints:
น่าจะต้องใช้ for loop วนเก็บข้อมูล
```
...
```text
Step 1: อ่าน input 4 บรรทัด a b d r
Step 2: ใช้ for loop ว่าถ้า i อยู่ในระหว่าง a b +1 เพราะถ้าไม่บวกหนึ่งมันจะไปหยุดทำตรงเลข b
Step 3: ตรวจว่า i % d == r หรือไม่
Step 4: ถ้าจริง ให้ +1 สะสมใน result
Step 5: พิมพ์ result เมื่ออกจาก loop
```
...
```text
ใช้เหมือนวิธีแรกเลยไม่ได้มีการปรับเปลี่ยนอะไร
```
