...
```text
โจทย์ให้ฉันคํานวณค่าธรรมเนียมการส่งพัสดุจะต้องเลือกต้นทาง - ปลายทาง และ น้ำหนักของพัสดุที่จะส่ง

Input:
บรรทัดแรก: ให้รับอินพุตสองตัว ต้นทาง - ปลายทาง
บรรทัดสอง: รับน้ำหนัก

Output:
แสดงผลราคาการนําส่งพัสดุที่ได้จากข้อมูลนําเข้าต้องเป็นทศนิยมสองตำแหน่งด้วย หากเส้นทางที่ระบุไม่อยู่ในตารางให้แสดงข้อความว่า Error 

Constraints:
ผลลัพธ์ต้องมีทศนิยมครบตามที่กำหนด — พิมพ์ 70 แทน 70.00 จะถือว่าผิด
```
...
```text
Step 1: รับค่าขาไป-ขากลับ และจำนวนเต็มน้ำหนัก
Step 2: if-else รันตรวจตามค่าที่ตารางให้มา 
Step 3: ถ้าเข้าข่ายกรณีตามตรางให้ผิดค่าส่งที่คำนวนแล้ว
Step 4: ถ้าไม่ พิมพ์ "Error"
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก เพราะผมคิดว่ามันไม่น่าจะมีปัญหาอะไร และเมื่อลองแต่ละเทสเคสก็ไม่ติดปัญหาใดๆ
```
...
```text
ทดสอบระบบ ที่การคำนวนไม่อยู่ในเงื่อนไข
```
