...
| OJ submission ID, if submitted | 	565669 |
| OJ status | Pass |
...
```text
ผมติดเทสเคสแล้วนึกไม่ออกว่าเป็นเพราะอะไรเลยให้ ai ช่วย debug ก็เลยทำให้รู้ว่าโค้ดผมมีข้อบกพร่องคือมันคำนวนออกมาแล้วอาจจะได้ติดลบ ทั้งที่โจทย์บอกว่าเป็น 0-10 ai เลยแน่นำให้ผมเพิ่ม max(0, เข้าไปข้างหน้าก่อนคำนวนเพื่อที่ถ้าผมคำนวนออกมาเป็นติดลบ ตรงนี้ก็จะบอกให้เอาค่าสูงสุดซึ่งเป็น 0
```
...
```text
ช่วยให้ผมสังเกตว่าโค้ดผมมีข้อบกพร่องที่ทำให้มันทำงานไม่ตรงโจทย์
```
...
```text
แก้ไขแค่เพิ่ม max(0, ไปข้างหน้าการหาผลรวมตามตำแนะนำของ ai
```
...
```text
ได้เรียนรู้วิธีการกำหนดเอาค่าสูงสุดในจำนวน ใช้เมื่อกรณีแบบนี้ก็ได้คือถ้าไม่อยากให้ค่ามันต่ำเกินจากนี้
```
