...
```text
โจทย์ให้ผมรับค่าว่าน้ำหมึกจะท่วมบ้านที่ต้องการคำนวณวินาทีที่เท่าไหร่

Input:
โปรแกรมรับจำนวนเต็ม s คือวินาที/ตร.หน่วย n คือจำนวนบ้านที่ต้องการคำนวณ
รับ x และ y ตามจำนวน n

Output:
วินาทีแบบปัดขึ้นของบ้านแต่ละหลัง

Constraints:
จะต้องปัดเลขขึ้นเป็นวินาทีเต็มถัดไปเช่นถ้า 1.3 ให้เป็น 2
```
...
```text
Step 1: รับจำนวน s และ n
Step 2: ตรวจว่า n > 0 ให้ รับ x, y ตามจำนวน n
Step 3: คำนวนหาระยะและวินาที
Step 4: output วินาทีแบบปัดขึ้น

ก่อนถาม ai ผมไม่มั่นใจเท่าไหร่ว่าต้องคำนวนยังไง
```
...
```text
ก็ทำเหมือนแบบแรกแต่สูตรคำนวนจาก ai มา และได้วิธีการใช้ append นำข้อมูลที่คำนวนแล้วเข้าไปเก็บไว้ใน list แล้วดึงมา output ทีเดียว
```
...
```text
ทดสอบว่าถ้าเปลี่ยนค่าทุกอย่างยังทำงานได้ปกติ
```
